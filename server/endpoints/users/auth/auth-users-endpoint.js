...
module.exports = function(req, res, next) {
  ...
  if (!req || !res || !req.body) {
    return jsonUtils.returnError(prop.config.http.bad_request, i18n.__('validation').auth_users_invalid_request_parameters, 'AuthenticationService', next);
  }
  ...
  UserService.findByNameAndPassword(username, password, function(err, userFromDB) {
    if (err) {
      return jsonUtils.returnError(prop.config.http.internal_server_error, i18n.__('validation').auth_users_not_found, 'AuthenticationService', next, err);

    } else {
      if (!userFromDB || !userFromDB.username) {
        return jsonUtils.returnError(prop.config.http.unauthorized, i18n.__('validation').auth_users_not_have_permission, 'AuthenticationService', next);
      } else {
        try {
          generateToken(userFromDB, function(err, token) {
            if (err) {
              return jsonUtils.returnError(prop.config.http.internal_server_error, i18n.__('validation').auth_users_error_to_generate_token, 'AuthenticationService', next, err);
            } else {
              return jsonUtils.returnSuccess(null, token, res, next);
            }
          });
        } catch (err) {
          return jsonUtils.returnError(prop.config.http.internal_server_error, i18n.__('internal_error').auth_users_fail_to_generate_token, 'AuthenticationService', next);
        }
      }
    }
  });
